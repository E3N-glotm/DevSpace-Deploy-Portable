...
当前仓库和二进制 Release 已公开。`ngrok.exe` 仍受 ngrok 当前服务条款约束，不属于本仓库 MIT License。**维护者应尽快确认当前随包分发方式获得明确授权，或改为用户首次启用 ngrok 时从官方来源下载并校验版本与 SHA-256。** 在完成该许可确认或下载式迁移前，不应把本仓库的 MIT License 解读为对 ngrok Agent 的再分发授权。
