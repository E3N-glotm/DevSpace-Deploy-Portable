...
`DEVSPACE_PERMISSION_PROFILE` controls host access independently of the tool
surface.

| Value | Behavior |
| --- | --- |
| `workspace` | Default. Keeps DevSpace file and working-directory resolution inside the opened workspace and presents conservative coding-workflow command guidance. |
| `full-access` | Allows any path accessible to the current operating-system user and explicitly authorizes arbitrary commands, SSH/network operations, credential helpers, shell file mutation, and interactive or persistent processes. It does not elevate to administrator, SYSTEM, or bypass UAC/ACLs. |
| `custom` | Uses the individual capability variables below. |

Custom capability variables:

| Variable | Purpose |
| --- | --- |
| `DEVSPACE_ALLOW_EXTERNAL_PATHS` | Permit absolute paths and working directories outside the opened workspace. |
| `DEVSPACE_ALLOW_ARBITRARY_COMMANDS` | Declare arbitrary host commands as owner-authorized in MCP instructions and tool descriptions. |
| `DEVSPACE_ALLOW_SHELL_MUTATION` | Declare shell-based file mutation as owner-authorized. |
| `DEVSPACE_ALLOW_NETWORK_ACCESS` | Declare network and SSH/SCP/SFTP operations as owner-authorized. |
| `DEVSPACE_ALLOW_CREDENTIAL_ACCESS` | Declare local credential API access as owner-authorized. |
| `DEVSPACE_ALLOW_INTERACTIVE_PROCESSES` | Permit `tty=true` process sessions. |
| `DEVSPACE_ALLOW_PERSISTENT_PROCESSES` | Permit commands to remain active after the first yield window and return a `sessionId`. |

Path, PTY, and persistent-session controls are enforced by the server. Command
categories are authorization declarations surfaced to the MCP client; a general
shell cannot be safely subdivided with a reliable string blacklist. Use the
`workspace` profile or disable arbitrary commands when a hard shell boundary is
required.
