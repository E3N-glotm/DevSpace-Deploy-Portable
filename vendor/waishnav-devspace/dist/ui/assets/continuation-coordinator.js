...
function visibleContinuationTrigger(task, deliveryToken) {
  ...
  return isChinese()
    ? [
      "继续。@DevSpace MCP 像人工发送“继续”一样持续完成未完成任务，不要用文字状态代替工作。",
      handshakeToken ? `deliveryToken：${handshakeToken}` : "",
      handshakeToken
        ? "首个 DevSpace 调用：continuation_task action=status，携带该 deliveryToken，不设置 manualTakeover；随后像人工发送“继续”一样直接做实质工作。不要用“继续”“继续处理中”或状态占位结束本轮；达到真实阶段边界时先调用 turn-complete，再给出简短可见进展摘要。若里程碑仍未完成，随后应继续自动续下一轮。"
        : "首个 DevSpace 调用：continuation_task action=status，不设置 manualTakeover；随后像人工发送“继续”一样直接做实质工作。不要用“继续”“继续处理中”或状态占位结束本轮；达到真实阶段边界时先调用 turn-complete，再给出简短可见进展摘要。若里程碑仍未完成，随后应继续自动续下一轮。",
    ].filter(Boolean).join("\n")
    : [
      "Continue. @DevSpace MCP keep completing the unfinished task exactly like a manual 'continue'; do not substitute prose status for work.",
      handshakeToken ? `deliveryToken: ${handshakeToken}` : "",
      handshakeToken
        ? "First DevSpace call: continuation_task action=status with this deliveryToken and without manualTakeover. Then, while any runnable milestone remains, output only substantive DevSpace tool calls: no 'continue', 'still working', progress/status prose, or final. Stop only at task terminal state, a genuine external blocker, or Host truncation."
        : "First DevSpace call: continuation_task action=status without manualTakeover. Then, while any runnable milestone remains, output only substantive DevSpace tool calls: no 'continue', 'still working', progress/status prose, or final. Stop only at task terminal state, a genuine external blocker, or Host truncation.",
    ].filter(Boolean).join("\n");
}
...
function continuationContext(task, workspaceId, reason) {
  ...
  const lines = [
    ...
    "The immediate action after status must be substantive DevSpace work. Do not use progress/status placeholders, '继续', '继续处理中', 'still working', promises, or an empty assistant final as a yield boundary. Keep invoking substantive DevSpace tools while the current stage is actively runnable. Host rendering of text between tool calls is not reliable, so do not depend on in-turn prose for progress visibility; the reliable user-visible checkpoint is the stage summary after a legal turn-complete. Never reveal private chain-of-thought.",
    "Call continuation_task status first. If the visible synthetic user-role request carries a one-time deliveryToken, echo that exact token on this first status call and omit manualTakeover; the runtime consumes it immediately when the turn-origin/ACK handshake succeeds. If no token was supplied, the runtime may compatibly claim a server-owned expected generation when the Host provides equivalent origin binding. Never invent, search for, or reuse a token after the first successful status. Then continue substantive work with the same full Host reasoning budget and sustained execution semantics as a manual 'continue': keep reading, editing, executing, validating, and polling owned long-running processes across multiple milestones. A synthetic resumed turn must perform at least four substantive DevSpace operations after its ACK before any model-owned completion control can be considered, but four operations are only an anti-idle floor and are never a target duration or an automatic reason to stop. When the resumed model reaches a genuine coherent stage boundary after sufficient sustained work, it may call continuation_task action=turn-complete even when later runnable milestones remain. That accepted control call authorizes a concise visible progress summary for the just-finished stage and arms the next automatic continuation for remaining milestones. If work becomes genuinely non-runnable, persist that state with checkpoint waitingExternal=true (or explicit pause/cancel/fail) instead of manufacturing a boundary. Synthetic duration is never a fixed number of minutes or a learned Host-budget percentage. Reuse the conversation-lifetime taskId and existing process/workspace state. Synthetic continuations reuse the current visible milestone-card generation while the required milestone set is unchanged. If and only if a status/checkpoint reports milestoneCardRequired/reanchorRequired because the synthetic checkpoint changed the required milestone set, issue continuation_anchor exactly once for that new generation; otherwise never create a duplicate card.",
    ...
    "Never end an automatically resumed turn with prose such as '继续', '继续处理中。', '继续处理。', 'still working', 'I will continue', a status-only sentence, or an empty final. There is no background model execution after a plain assistant final. Before ANY user-visible final from a synthetic turn, the final DevSpace control call must legally permit that final. If finalResponseAllowed=false or syntheticWorkMustContinue=true, either continue substantive work or, only after a genuine coherent stage boundary and sufficient sustained work, call turn-complete. A failed command/test or a small number of quick calls is not such a boundary. Once turn-complete is accepted, provide a concise visible progress summary; if milestones remain, the next synthetic continuation should be scheduled automatically. If the current cached schema does not expose turn-complete, use continuation_task action=checkpoint with note=atcc-turn-complete, which is subject to the same work floor and stage-boundary requirement.",
  ];
  return lines.join("\n");
}
...
function activeSyntheticExecutionContext(task, workspaceId, reason) {
  return [
    ...
    "If deliveryOwner=synthetic-active and a runnable milestone remains, a plain/empty final is illegal. Continue substantive work; at a genuine coherent stage boundary after sufficient sustained work, call turn-complete as the final DevSpace control call, then emit a concise visible progress summary. If milestones remain, that boundary should arm the next automatic continuation.",
  ].join("\n");
}
