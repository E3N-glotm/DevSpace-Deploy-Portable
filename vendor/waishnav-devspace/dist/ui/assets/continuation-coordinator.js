...
function visibleContinuationTrigger(task, deliveryToken) {
  ...
  return isChinese()
    ? [
      ...
      handshakeToken
        ? "首个 DevSpace 调用：continuation_task action=status，携带该 deliveryToken，不设置 manualTakeover；status 成功后的第二个动作必须直接调用实质 DevSpace 工具继续任务，在第一次实质工作前禁止回复“继续处理中”、状态说明或稍后继续。"
        : "首个 DevSpace 调用：continuation_task action=status，不设置 manualTakeover；status 成功后的第二个动作必须直接调用实质 DevSpace 工具继续任务，在第一次实质工作前禁止回复“继续处理中”、状态说明或稍后继续。",
    ].filter(Boolean).join("\n")
    : [
      "@DevSpace MCP continue the unfinished task. Continue real work exactly like a manual 'continue'.",
      handshakeToken ? `deliveryToken: ${handshakeToken}` : "",
      handshakeToken
        ? "First DevSpace call: continuation_task action=status with this deliveryToken and without manualTakeover. The second action after a successful status MUST be a substantive DevSpace tool call; before that first substantive operation, do not reply with 'still working', a status/progress message, or a promise to continue later."
        : "First DevSpace call: continuation_task action=status without manualTakeover. The second action after a successful status MUST be a substantive DevSpace tool call; before that first substantive operation, do not reply with 'still working', a status/progress message, or a promise to continue later.",
    ].filter(Boolean).join("\n");
}
...
function continuationContext(task, workspaceId, reason) {
  ...
  const lines = [
    ...
    "Before the first successful post-ACK substantive DevSpace operation, emit no user-visible assistant text at all: no progress/status placeholder and no promise to continue. The immediate action after status must be substantive DevSpace work. During sustained automatic work *after real work has begun*, emit occasional concise user-visible progress updates so the user can tell that the resumed turn is actively working. These are in-turn progress messages only: do not reveal private chain-of-thought, do not treat a progress update as a final response, and continue substantive DevSpace work immediately afterwards while runnable milestones remain.",
