...
function visibleContinuationTrigger(task, deliveryToken) {
  ...
  return isChinese()
    ? [
      "继续。@DevSpace MCP 继续完成未完成任务，像人工发送“继续”一样持续执行。",
      handshakeToken ? `deliveryToken：${handshakeToken}` : "",
      handshakeToken
        ? "首个 DevSpace 调用：continuation_task action=status，携带该 deliveryToken，不设置 manualTakeover；随后从返回的执行位置继续工作。"
        : "首个 DevSpace 调用：continuation_task action=status，不设置 manualTakeover；随后从返回的执行位置继续工作。",
      "如果首个 status 调用只返回工具内部错误或连接错误、且尚未 ACK，同一轮立即重试同一个 status 调用（最多 3 次）；不要因为一次传输错误结束本轮。",
    ].filter(Boolean).join("\n")
    : [
      "Continue. @DevSpace MCP continue the unfinished task exactly like a manual 'continue'.",
      handshakeToken ? `deliveryToken: ${handshakeToken}` : "",
      handshakeToken
        ? "First DevSpace call: continuation_task action=status with this deliveryToken and without manualTakeover; then continue from the returned execution state."
        : "First DevSpace call: continuation_task action=status without manualTakeover; then continue from the returned execution state.",
      "If that first status call only returns a tool-internal or connector error before ACK, immediately retry the same status call in this turn (up to 3 attempts); never end the turn because of one transport failure.",
    ].filter(Boolean).join("\n");
}
...
function continuationContext(task, workspaceId, reason) {
  ...
  const lines = [
    ...
    "Call continuation_task status first. If supplied a one-time deliveryToken, echo that exact token and omit manualTakeover; the runtime consumes it immediately. Never reuse it after ACK. If the Host/tool connector returns only an internal/transport failure before the ACK reaches DevSpace, retry that exact first status call in the same turn up to three attempts; do not emit a final after one transient transport failure. The immediate next action after ACK is substantive DevSpace work, not a final or a promise.",
    "Use the same full Host reasoning budget and sustained execution semantics as a manual 'continue': keep reading, editing, executing, validating, and polling owned long-running processes across multiple milestones. A running process or partial test batch is unfinished work; poll it to its result and continue the next runnable step. Do not confuse background process progress with model work.",
    "A synthetic turn requires at least four substantive DevSpace operations after its ACK as an anti-empty guard. Four operations are only an anti-idle floor and are never a target duration or an automatic reason to stop. While any milestone remains runnable, turn-complete is rejected with synthetic-turn-runnable-milestones-remain unless the server reports preCutoffHandoffRequired=true. Only then sign turn-complete, preserve the exact next operation, and give a concise progress summary. This is a Host cutoff handoff, not a model-chosen stage boundary. Synthetic duration is never a fixed number of minutes or a learned Host-budget percentage.",
    "Complete the task when all required work is verified. Persist waitingExternal=true only for a genuine unavailable external dependency; a running test/process, one failed command, a checkpoint, or a desire to summarize is not an external blocker. Never use status-only, placeholder, promise, or empty finals. No model work continues after a final response.",
    "Reuse the lifetime taskId and current workspace/process state. Reuse the current milestone card unless status/checkpoint explicitly requests a new generation. If staleSyntheticTurn=true, suppressVisibleFinal=true, superseded=true, or reason=synthetic-continuation-superseded is returned, stop immediately and silently: perform no substantive work, make no further DevSpace calls, and emit no user-visible status/final. For an older schema, checkpoint note=atcc-turn-complete uses exactly the same gate as turn-complete.",
  ];
  return lines.join("\n");
}
...
function activeSyntheticExecutionContext(task, workspaceId, reason) {
  return [
    ...
    "If deliveryOwner=synthetic-active and a runnable milestone remains, continue substantive work in this same turn. Poll running processes to their result. A partial test batch, checkpoint, or operation count is not a final boundary. Sign turn-complete only when the server reports preCutoffHandoffRequired=true; otherwise complete all work or persist a genuine external blocker.",
  ].join("\n");
}
