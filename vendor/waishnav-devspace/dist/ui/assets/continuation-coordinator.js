...
function visibleContinuationTrigger(task, deliveryToken) {
  ...
  return isChinese()
    ? [
      ...
      handshakeToken
        ? "首个 DevSpace 调用：continuation_task action=status，携带该 deliveryToken，不设置 manualTakeover；随后只要还有可运行里程碑，就连续调用实质 DevSpace 工具。期间不要输出“继续”“继续处理中”或任何进度/状态文字，也不要结束本轮；只有任务终态、真实外部阻塞或 Host 截断才停止。"
        : "首个 DevSpace 调用：continuation_task action=status，不设置 manualTakeover；随后只要还有可运行里程碑，就连续调用实质 DevSpace 工具。期间不要输出“继续”“继续处理中”或任何进度/状态文字，也不要结束本轮；只有任务终态、真实外部阻塞或 Host 截断才停止。",
    ].filter(Boolean).join("\n")
    : [
...
function continuationContext(task, workspaceId, reason) {
  ...
  const lines = [
    ...
    "While this synthetic turn still has any runnable milestone, emit no user-visible assistant prose at all between tool calls. The immediate action after status must be substantive DevSpace work, and after every successful substantive result the next model output must again be a substantive DevSpace tool call. Do not emit progress/status placeholders, '继续', '继续处理中', 'still working', promises, summaries, or other prose while the synthetic Task Contract remains runnable. The milestone card and native tool activity are the live progress surface; prose is allowed only after the Task Contract reaches a legal terminal/non-runnable boundary. Never reveal private chain-of-thought.",
    ...
    "Never end or interrupt an automatically resumed runnable turn with prose such as '继续', '继续处理中。', '继续处理。', 'still working', 'I will continue', a progress summary, or a promise. There is no background model execution after a final assistant message, and the current Host does not reliably distinguish an intended in-turn progress sentence from a real assistant final. Therefore a runnable synthetic turn is tool-only: after each tool result, immediately issue the next substantive DevSpace tool call. A failed command/test or a small number of quick tool calls is not a legitimate yield boundary. Before ANY user-visible final from a synthetic turn, the final DevSpace control call must legally permit that final. If finalResponseAllowed=false or syntheticWorkMustContinue=true, a plain final is forbidden even after four or more substantive operations. If turn-complete is rejected with synthetic-turn-runnable-milestones-remain, continue substantive work instead of answering the user. If the current cached schema does not expose turn-complete, use continuation_task action=checkpoint with note=atcc-turn-complete, which is subject to the same gate. Only all milestones complete, an explicit non-runnable checkpoint/pause/cancel/fail, or a genuine Host truncation may end an unfinished synthetic work window.",
  ];
  return lines.join("\n");
}
...
function activeSyntheticExecutionContext(task, workspaceId, reason) {
  return [
    ...
    "If deliveryOwner=synthetic-active and a runnable milestone remains, this assistant turn is NOT at a legal prose/final boundary. The next model output after each completed tool result MUST be another substantive DevSpace tool call unless the Task Contract becomes terminal/non-runnable or the Host itself truncates execution. Emit no progress/status prose while runnable.",
  ].join("\n");
}
