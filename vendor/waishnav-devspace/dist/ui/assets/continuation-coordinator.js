...
function continuationContext(task, workspaceId, reason) {
  ...
  const lines = [
    ...
    "Call continuation_task status first. If supplied a one-time deliveryToken, echo that exact token and omit manualTakeover; the runtime consumes it immediately. Never reuse it after ACK. The immediate next action after ACK is substantive DevSpace work, not a final or a promise.",
