...
// Hidden sender protocol compatibility gate. Increment only when the App-only
...
// Epoch 12 and 13 use the same sender wire and authorize-before-send contract.
...
function preFinalBarrier(outcome) {
    const task = outcome?.task;
    if (!task)
        return undefined;
    const remainingMilestones = Array.isArray(outcome.remainingMilestones)
    ...
    const preFinalControlRequired = outcome.preFinalControlRequired === true;
    const mustContinueSameTurn = Boolean(outcome.taskIncomplete)
        && (preFinalControlRequired || !finalResponseAllowed);
    return {
        protocol: "devspace-pre-final-barrier-v1",
        taskId: task.id,
        turnLeaseId: task.turnLeaseId,
        syntheticActive: task.deliveryOwner === "synthetic-active",
        taskIncomplete: Boolean(outcome.taskIncomplete),
...
function preFinalBarrierText(barrier) {
    if (!barrier?.mustContinueSameTurn)
        return undefined;
    return [
        ...
        ...(barrier.syntheticActive ? [
            "UNFINISHED SYNTHETIC TURN: A PLAIN OR EMPTY FINAL IS FORBIDDEN WHILE MILESTONES REMAIN.",
            "Continue substantive DevSpace work in this same turn; poll running work to its result and then execute the next runnable step. Partial test progress, checkpoints, and operation counts are not final boundaries.",
            "Only when the server reports preCutoffHandoffRequired=true may the FINAL DevSpace control call be continuation_task action=turn-complete. After accepted Host cutoff handoff, give a concise progress summary. Otherwise finish the task or persist a genuine external blocker.",
            barrier.requiredBeforeFinal,
        ] : [
...
function withContinuationSenderCapability(result, taskContractOutcome, definition, options = {}) {
    if (!result)
        return result;
    ...
    const includeLegacyOutputTemplate = options.includeLegacyOutputTemplate !== false;
        ...
        _meta: {
            ...
            ...(resourceUri
                ? {
                    ...
                    ...(includeLegacyOutputTemplate ? { "openai/outputTemplate": resourceUri } : {}),
                }
                : {}),
...
// Keep exact-name discovery selective. Shared protocol prose must not make
// every ordinary tool a lexical match for the task-control entry point.
function continuationDiscoveryDescription(description) {
    return String(description ?? "").replaceAll("continuation_task", "task-control tool");
}
function syntheticAckWorkTicket(input, outcome) {
    if (input.action !== "status" || input.manualTakeover || input.readOnlyStatus
        || input.coordinatorInstanceId || outcome.accepted !== true
        || !["continuation-resume-acknowledged", "server-owned-expected-generation-claimed"].includes(outcome.reason))
        return outcome;
    const task = outcome.task;
    const remainingMilestones = Array.isArray(outcome.remainingMilestones)
        ? outcome.remainingMilestones
    ...
    const executionContract = {
        ...
        turnLeaseId: task?.turnLeaseId,
        taskIncomplete: Boolean(outcome.taskIncomplete),
        continueRequired: outcome.continueRequired !== false,
        continueInSameTurn: outcome.continueInSameTurn === true,
        syntheticWorkMustContinue: outcome.syntheticWorkMustContinue === true,
        preFinalControlRequired: outcome.preFinalControlRequired === true,
        finalResponseAllowed: outcome.finalResponseAllowed !== false,
        mustContinueSameTurn: Boolean(outcome.taskIncomplete)
            && (outcome.syntheticWorkMustContinue === true
                || outcome.preFinalControlRequired === true
        ...
        // model already receives task.resumeContext in the same payload.  The
        // contract retains the concise resume instruction below, while the
        // durable capsule stays directly addressable and lossless.
        resumeInstruction: resumeContext?.resumeInstruction
            ?? "Resume from the latest durable execution operation. Do not redo workspace/root-cause discovery solely to reconstruct context.",
        requiredBeforeFinal,
    };
    // Projection only: retain the complete evidence/ownership history and
    // generation CAS in RuntimeState, but restore the actionable milestone
    // ledger that pre-dev71 full ACKs exposed to the resumed model. Generation
    // 12 proved that a generic "SUBSTANTIVE_DEVSPACE_WORK" ticket can be ACKed
    // and then abandoned before the first real tool call. Keep this bounded by
    // excluding evidence while making the exact next unit of work unambiguous.
    return {
        accepted: true,
        ...
        task: {
            ...
            state: task.state,
            assistantTurnOwner: task.assistantTurnOwner,
            turnLeaseId: task.turnLeaseId,
            deliveryGeneration: task.deliveryGeneration,
            requiredMilestones,
            completedMilestones,
            nextMilestone,
            ...(nextExecutableStep ? { nextExecutableStep } : {}),
        ...
        remainingMilestones,
        nextRequiredMilestones: remainingMilestones,
        reanchorRequired: outcome.reanchorRequired === true,
        taskIncomplete: outcome.taskIncomplete,
        continueInSameTurn: outcome.continueInSameTurn,
        syntheticWorkMustContinue: outcome.syntheticWorkMustContinue,
        finalResponseAllowed: outcome.finalResponseAllowed,
        preFinalControlRequired: outcome.preFinalControlRequired,
        requiredBeforeFinal,
    };
}
function registerAppTool(server, name, definition, handler) {
    const guardedDefinition = continuationTaskContractsEnabled
        ...
        ? {
            ...definition,
            description: continuationDiscoveryDescription(`${CONVERSATION_CARD_PRECONDITION} ${String(definition?.description ?? "").trim()}`.trim()),
            // Data-plane tools expose only the compact pre-final gate. Durable
            // Task Contract/history/resume state stays server-side and is read
            // through continuation_task. Re-emitting the full mutable task on
            // every read/exec/edit made each synthetic step grow with its own
            // history and consumed the resumed turn after only one or two ops.
            ...(definition.outputSchema ? {
                outputSchema: {
                    ...definition.outputSchema,
                    devspacePreFinalBarrier: z.unknown().optional(),
                    anchorToolCallRequired: z.boolean().optional(),
                },
            } : {}),
        }
        : definition;
    return registerExtAppTool(server, name, guardedDefinition, async (input, context = {}) => {
        ...
        try {
            ...
            if (continuationTaskContractsEnabled && !continuationControlCall && conversationScopeId && structuredRuntimeState) {
                ...
                if (!setupOnlyCall) {
                    const resumeContext = structuredRuntimeState.recordContinuationResumeOperation({
                        conversationScopeId,
                        ...(workspaceId ? { workspaceId } : {}),
                        operation: continuationResumeOperation(name, input, result),
                    });
                    if (resumeContext && taskContractOutcome?.task) {
                        // Keep the same successful tool response useful to the
                        // current model while also making the exact working set
                        // durable for a later synthetic turn.
                        taskContractOutcome.task.resumeContext = resumeContext;
                    }
                }
        ...
        const extraContent = [];
        const barrier = preFinalBarrier(taskContractOutcome);
        const barrierText = preFinalBarrierText(barrier);
        // The short barrier deliberately precedes the ordinary tool payload.
        // Long Task Contract prose appended after a successful tool result can
        // be de-prioritized by the model when it is ready to answer. Putting a
        // compact, machine-readable stop condition first makes the legal next
        // action salient without inventing an unsafe silence/lease heuristic.
        const barrierContent = barrierText ? [textBlock(barrierText)] : [];
        ...
        const senderCapableResult = withContinuationSenderCapability(result, taskContractOutcome, guardedDefinition, {
            includeLegacyOutputTemplate: name !== "continuation_anchor",
        });
        if (!supervisorDirective?.reanchorRequired) {
            if (extraContent.length === 0 && barrierContent.length === 0) return senderCapableResult;
            return {
                ...senderCapableResult,
                content: [...barrierContent, ...(Array.isArray(senderCapableResult?.content) ? senderCapableResult.content : []), ...extraContent],
                structuredContent: senderCapableResult?.structuredContent && typeof senderCapableResult.structuredContent === "object"
                    ? { ...senderCapableResult.structuredContent, devspacePreFinalBarrier: barrier }
        ...
        return {
            ...senderCapableResult,
            content: [...barrierContent, ...(Array.isArray(senderCapableResult?.content) ? senderCapableResult.content : []), ...extraContent, textBlock(maintenanceText)],
            structuredContent: senderCapableResult?.structuredContent && typeof senderCapableResult.structuredContent === "object"
                ? { ...senderCapableResult.structuredContent, devspacePreFinalBarrier: barrier }
                : senderCapableResult?.structuredContent,
        };
...
// Control replies cross both the server's Zod validator and the Host's JSON
// Schema validator. Keep shared outcome fields explicit: an undeclared field
// can make the Host reject a successful bind after its lease was persisted.
function continuationTransportOutputFields() {
    return {
        ...
        anchorMountVerificationPending: z.boolean().optional(),
        senderInstanceId: z.string().optional(),
        lastSenderHeartbeatAt: z.string().optional(),
        lastUiHeartbeatAt: z.string().optional(),
        senderRebindReleasedClaim: z.boolean().optional(),
        senderRebindReleasedGeneration: z.number().int().optional(),
        preFinalControlRequired: z.boolean().optional(),
        requiredBeforeFinal: z.string().optional(),
        preCutoffHandoffRequired: z.boolean().optional(),
        preCutoffHandoffAtMs: z.number().nonnegative().optional(),
        preCutoffHandoffMarginMs: z.number().nonnegative().optional(),
        preCutoffDeliveryMedianMs: z.number().nonnegative().optional(),
        elapsedTurnMs: z.number().nonnegative().optional(),
        nextRequiredMilestones: z.array(z.string()).optional(),
        milestoneCardRequired: z.boolean().optional(),
        conversationLifetimeSingleton: z.boolean().optional(),
        readOnlyStatus: z.boolean().optional(),
        retryExisting: z.boolean().optional(),
        deliveryDueAt: z.string().optional(),
        deliveryAckRetry: z.boolean().optional(),
        deliveryDiagnostics: z.object({
            generation: z.number().int().optional(),
            state: z.string().optional(),
            deliveredAt: z.string().nullable(),
            turnAckedAt: z.string().nullable(),
            ackLeaseExpiresAt: z.string().nullable(),
            ackLeaseExpired: z.boolean(),
        ...
        continuationAnchor: z.literal(true).optional(),
...
function compactResumeText(value, max = 1200) {
...
function continuationResumeOperation(name, input, result) {
    const structured = result?.structuredContent ?? {};
    const nestedProcess = structured?.process ?? {};
    const files = Array.isArray(structured?.files)
        ? structured.files.slice(0, 20).map((file) => ({
            path: compactResumeText(file?.path, 500),
            previousPath: compactResumeText(file?.previousPath, 500),
            operation: compactResumeText(file?.operation, 80),
        }))
        : undefined;
    const command = name === "exec_command"
        ? displayCommand(input?.cmd, input?.argv)
        : undefined;
    const paths = Array.isArray(input?.paths)
        ? input.paths.slice(0, 20).map((path) => compactResumeText(path, 500)).filter(Boolean)
        : undefined;
    return redactValue({
        tool: name,
        success: result?.isError !== true,
        workspaceId: compactResumeText(input?.workspaceId ?? structured?.workspaceId, 200),
        path: compactResumeText(input?.path, 500),
        paths,
        workingDirectory: compactResumeText(input?.workingDirectory, 500),
        command: compactResumeText(command, 1600),
        pattern: compactResumeText(input?.pattern ?? input?.query, 500),
        ...
        processHandle: compactResumeText(structured?.processHandle ?? nestedProcess?.processHandle, 240),
        running: typeof structured?.running === "boolean" ? structured.running
            : typeof nestedProcess?.running === "boolean" ? nestedProcess.running : undefined,
        exitCode: Number.isFinite(Number(structured?.exitCode)) ? Number(structured.exitCode)
            : Number.isFinite(Number(nestedProcess?.exitCode)) ? Number(nestedProcess.exitCode) : undefined,
        files,
        resultSummary: compactResumeText(contentText(Array.isArray(result?.content) ? result.content : []), 1400),
    });
}
...
function workspaceAppAnchorUri(config) {
    return workspaceAppUri(config).replace(/\.html$/, "-continuation-anchor.html");
}
function workspaceAppGenerationUri(config, generation) {
    const baseUri = workspaceAppAnchorUri(config);
...
function workspaceAppResultMeta(config, generation) {
    // Keep the statically registered continuation_anchor descriptor on the
    // long-proven primary Workspace App URI, but let each successful visible
    // card result advertise a standards-first immutable generation URI. A live
    // ChatGPT conversation may retain the pre-upgrade descriptor/outputTemplate
    // and even the old App document across an MCP reconnect. Reusing that cache
    // key can therefore mount a fresh card with stale coordinator bytes. The
    // result-level ui.resourceUri gives the Host a new document identity for the
    // exact card generation without reviving dev15's second legacy
    // openai/outputTemplate, which previously produced an unmounted result shell.
...
function workspaceAppSurfaceBootstrap(resourceUri) {
    const uri = String(resourceUri ?? "");
    const anchorMatch = /-continuation-anchor(?:-g(\d+))?\.html$/.exec(uri);
    const generation = Number(anchorMatch?.[1] || 0);
    return {
        kind: anchorMatch ? "continuation-anchor" : "workspace",
        ...(Number.isInteger(generation) && generation > 0 ? { anchorMountGeneration: generation } : {}),
    };
}
function workspaceAppHtml(config, resourceUri = workspaceAppUri(config)) {
    ...
    const surfaceBootstrap = JSON.stringify(workspaceAppSurfaceBootstrap(resourceUri));
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
${inlineStyles}
    ...
    <script>
      // Tool-input/tool-result notifications are optional Host lifecycle
      // messages. Keep the surface identity in the immutable MCP resource URI
...
function registerRuntimeStateTools(server, config, workspaces, runtimeState, fileWatches, permissionRules, processSessions, remoteAgents, continuationSupervisor) {
    if (config.features?.continuationGuard) {
        ...
        const runContinuationSenderBridge = (input, context = {}) => {
            ...
            // Record only decision points, never capabilities or message text.
            // A successful bind/heartbeat is not evidence that claim was tried.
            if ((action === "bind" && outcome?.readyGeneration) || action === "claim") {
                runtimeState.appendEvent({
                    kind: action === "bind" ? "continuation-sender-ready-observed" : "continuation-sender-claim-result",
                    subject: failureScopeId,
                    payload: {
                        senderInstanceId: input.senderInstanceId,
                        observedAssetRevision: input.senderAssetRevision,
                        accepted: outcome?.accepted === true,
                        reason: outcome?.reason ?? null,
                        generation: outcome?.readyGeneration ?? outcome?.generation ?? null,
                    },
                });
            }
            return outcome;
        ...
        registerAppTool(server, "continuation_anchor", {
            ...
            outputSchema: resultOutputSchema(continuationTransportOutputFields()),
        ...
        registerAppTool(server, "continuation_task", {
            ...
            description: "Persist and verify the single ChatGPT-thread lifetime DevSpace Task Contract across assistant turns, sequential user tasks, card generations and workspace switches. The first DevSpace call in every assistant turn must be status. On the first status of each manual/user turn, set manualTakeover=true exactly once. A synthetic resumed turn omits manualTakeover and, when its user-role handoff supplies a one-time deliveryToken, echoes that exact token on its first status to complete turn-origin/ACK binding; the token is consumed immediately after success. Hosts with equivalent out-of-band origin binding may still use the server-owned tokenless expected-generation claim as a compatibility path. completion-driven and resident use the Assistant Turn Completion Contract (ATCC): activity lease expiry may record SUSPECTED_STALL, but request silence, iframe heartbeat and ownership-lease expiry alone never authorize another Host turn. A single or unstable historical cutoff remains telemetry only. As a server-only fallback for Hosts that silently hard-cut the model without emitting teardown/timeout, at least two distinct tightly clustered confirmed cutoff observations may infer TIMED_OUT only after an adaptive post-cutoff margin, bound to the exact current turn lease and only when no model-originated request is in flight. A watched external process is independent of assistant-turn lifetime and does not block that recovery. Any substantive DevSpace request that survives the learned survival boundary invalidates the old cutoff regime and revokes an unconsumed inferred continuation. timeout-recovery remains strict and does not use this inference. Never manufacture samples or claim live acceptance from a simulation. Every status/control result may expose preFinalControlRequired and requiredBeforeFinal. Before ANY visible final in a DevSpace-using turn, inspect the latest Task Contract: when preFinalControlRequired=true or finalResponseAllowed=false, a plain final is forbidden. Continue substantive work; or, for manual turns at a genuine intentional stage boundary, make action=turn-complete the final DevSpace control call; synthetic turns with runnable milestones may sign only when the server reports preCutoffHandoffRequired=true; or persist a real external blocker with checkpoint waitingExternal=true. Never leave an incomplete RUNNING task in GENERATING because a connector failed, one milestone completed, a summary is ready, or the activity lease became SUSPECTED_STALL. A manual assistant stage that intentionally ends while milestones remain must call action=turn-complete after substantive work; if an already-open ChatGPT Host has cached an older schema that does not expose turn-complete, call action=checkpoint with the exact note atcc-turn-complete instead. That compatibility signature is routed through the identical ATCC current-turn and substantive-work gates and does not change ordinary checkpoint behavior. Do not sign completion merely because one command/test failed or because only a few quick tool calls have run: diagnose failures and continue runnable work in the same assistant turn. A matching verified Host teardown is an optional immediate confirmation fast path, not a requirement for ordinary ChatGPT finals. When teardown is absent, only an exact model-signed COMPLETION_REQUESTED turn lease may promote to COMPLETED after the bounded 8-second handoff grace and only when no model-originated DevSpace request is in flight. GENERATING silence cannot enter that model-signed path. Any later substantive DevSpace call revokes the pending request back to GENERATING, and manual takeover rotates the turn lease, so stale completion intent cannot fire later. A verified explicit Host timeout may independently record TIMED_OUT. Manual completion intent requires at least one substantive operation; a synthetic resumed turn requires at least four post-ACK substantive DevSpace operations before it may voluntarily sign an unfinished stage boundary. These are anti-idle floors only, not duration budgets or permission to stop. Both turn types otherwise use the same milestone-driven stopping rules and full Host reasoning budget; fixed minutes and learned Host-budget percentages never authorize the model itself to end early. Completing one milestone does not justify ending the turn while later runnable milestones remain. timeout Host signals are accepted only from the verified exact-turn anchor/sender authority; generic teardown and cancellation text cannot be promoted to timeout. resident combines persistent process monitoring with the same normal-completion, verified-timeout and narrow clustered-cutoff fallback contract. A running process never blocks recovery of an ended model turn, and a completed process never ends a live model turn. Process watchers run in the server independently of the card; stage-complete uses the normal model-owned turn-complete gates.",
            inputSchema: {
                ...
                completionDisposition: z.enum(["auto-resume", "await-user"]).optional().describe("turn-complete: auto-resume requests continued work; await-user is only for explicit user takeover and disables automatic delivery while preserving unfinished milestones. Cached schemas can use checkpoint note=atcc-await-user. Mark completed milestones before final; never request continuation merely to dismiss a stale final barrier."),
            ...
            const boundTaskConversationScopeId = input.taskId
                ? runtimeState.continuationTask({ action: "status", taskId: input.taskId, readOnlyStatus: true }).task?.conversationScopeId
                : undefined;
                ...
                if (task && workspaceId && handles.length > 0) {
                    ...
                    for (const processHandle of handles) {
                        try {
                            ...
                            const process = {
                                processHandle,
                                running: typeof snapshot.running === "boolean" ? snapshot.running : undefined,
                                exitCode: snapshot.exitCode,
                                signal: snapshot.signal,
                                status: snapshot.status,
                            };
                    ...
                    for (const processHandle of completedHandles) {
                        runtimeState.trackContinuationActivityProcess({
                            conversationScopeId, processHandle, running: false,
                        });
                    }
                }
                const refreshed = task
                    ? runtimeState.continuationTask({ action: "status", taskId: task.id, conversationScopeId, readOnlyStatus: true }).task
                    : undefined;
                const payload = {
                    task: refreshed,
                    accepted: Boolean(task),
                    wakeReady: Boolean(refreshed?.continuationWakePending),
                    watchedProcesses,
                };
                const result = JSON.stringify(payload, null, 2);
                return { content: [textBlock(result)], structuredContent: { result, ...payload } };
            ...
            const payload = syntheticAckWorkTicket(input, outcome);
            const result = JSON.stringify(payload, null, 2);
            const executionDirective = payload?.task?.workTicket === "synthetic-execution-v3"
                ? String(payload.requiredBeforeFinal ?? "").trim()
                : "";
            return {
                content: executionDirective
                    ? [textBlock(`DEVSPACE SYNTHETIC EXECUTION HANDOFF [P0]\n${executionDirective}`), textBlock(result)]
                    : [textBlock(result)],
                structuredContent: { result, ...payload },
            };
        });
...
function createMcpServer(config, workspaces, reviewCheckpoints, processSessions, localAgentProviders, incomingArtifactAdapters, runtimeServices) {
    ...
    const continuationAnchorResourceUri = workspaceAppAnchorUri(config);
    ...
    registerAppResource(server, "DevSpace Continuation Anchor", continuationAnchorResourceUri, workspaceResourceMetadata, async () => {
        await assertWorkspaceAppAssets();
        return workspaceAppResourceResult(config, continuationAnchorResourceUri);
    });
...
export function createServer(config = loadConfig(), options = {}) {
    ...
    const writeContinuationWake = (res, reason = "ready") => {
        ...
        try {
            ...
            // Force small SSE frames across intermediaries immediately. The
            // continuation wake is latency-sensitive but carries no authority;
            // buffering it until another chunk defeats the purpose and pushes
            // READY -> CLAIM back onto throttled iframe timers.
            res.flush?.();
            return true;
        }
        catch {
    ...
    // List-change notifications describe catalog changes; they are not a Host
    // turn-start or App-lifecycle API. dev83's probe produced successful sends
    // to 90+ sessions every five seconds without proving READY consumption.
    // Keep capability diagnostics, but do not broadcast metadata for task wakes.
    ...
    const continuationSupervisor = createContinuationSupervisorScheduler({
        ...
        onSweep: (sweep, reason) => {
            ...
            if (Array.isArray(sweep.deliveryAckRetryDue) && sweep.deliveryAckRetryDue.length > 0) {
                ...
                // Wake-only transport hint. The App must still refresh status
                // and win the sender CAS, which reuses the same logical
                // generation/token. This path therefore cannot manufacture a
                // duplicate milestone card or infer model completion from
                // silence.
        ...
        try {
            ...
            if (initializeRequest && initializedServer) {
                ...
                const protocolVersion = req.body?.params?.protocolVersion
                    ? String(req.body.params.protocolVersion)
                    : undefined;
                const clientCapabilities = initializedServer.server?.getClientCapabilities?.();
                const clientVersion = initializedServer.server?.getClientVersion?.();
                if (initializedSessionId) {
                    transports.updateMetadata(initializedSessionId, {
                ...
                runtimeState.appendEvent({
                    kind: "mcp.client.capabilities",
                    subject: initializedSessionId ?? null,
                    payload: {
                        sessionIdPrefix: sessionIdPrefix(initializedSessionId),
                        protocolVersion: protocolVersion ?? null,
                        clientVersion: clientVersion ?? null,
                        clientCapabilities: clientCapabilities ?? null,
                        samplingSupported: Boolean(clientCapabilities?.sampling),
                        samplingToolsSupported: Boolean(clientCapabilities?.sampling?.tools),
                    },
                });
...
export { collectWorkspacePreviews, nativeAttachmentContent, processToolResponse, redactDisplayArgv, reviewOperation, shouldAttachWidget, toolInvocationStatus, toolWidgetDescriptorMeta, workspaceAppAnchorUri, workspaceAppGenerationUri, workspaceAppHtml, workspaceAppResourceResult, workspaceAppResultMeta, workspaceAppUri };
