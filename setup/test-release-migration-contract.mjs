...
assert.match(workflow, /name: Resolve GitHub Release files/);
assert.match(workflow, /id: release_files/);
assert.match(workflow, /"DevSpacePortable-Windows-x64-\$env:VERSION\.zip"/);
assert.match(workflow, /"DevSpacePortable-Update-\*-to-\$env:VERSION\.zip"/);
assert.match(workflow, /"DevSpacePortable-Windows-x64-\$env:VERSION\.blockmap"/);
