...
try {
  ...
  assert.match(nativeSource, /class SessionDiffDialog/);
  assert.match(nativeSource, /FormBorderStyle = FormBorderStyle\.Sizable/);
  assert.match(nativeSource, /MaximizeBox = true/);
  assert.match(nativeSource, /ActionButton\("打开差异窗口"/);
  assert.match(nativeSource, /可自由调整窗口大小/);
  assert.match(nativeSource, /请在主窗口选择一个文件/);
  ...
  assert.match(nativeSource, /class ContentPreviewDialog/);
  assert.match(nativeSource, /ActionButton\("打开完整内容窗口"/);
  assert.match(nativeSource, /Panel editorScroll = new Panel \{ Dock = DockStyle\.Fill, AutoScroll = true/);
  assert.doesNotMatch(nativeSource, /NewGroup\("完整内容预览"\)/);
  assert.match(nativeSource, /Text = "显示其他"/);
