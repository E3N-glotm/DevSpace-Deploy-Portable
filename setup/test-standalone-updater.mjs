...
assert.match(updaterSource, /ReadStagedUpdateMode/);
...
try {
  ...
  assert.equal(report.incrementalApplyFullFallback, true);
  assert.equal(report.programCommitIndependentOfServiceRecovery, true);
  assert.equal(report.preApplyStopMustSucceed, true);
